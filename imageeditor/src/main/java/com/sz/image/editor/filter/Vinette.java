 ...
 */
public class Vinette extends Filter{

  private final String VINETTE = "vinette";
  ...
  public Vinette(Context context,String targetPath, int width, int height){
    mContext = context;
  ...
  public void assignFilterImage(TextView textView, ImageView imageView) {
    ...
    Glide.with(mContext)
            ...
            .bitmapTransform(new VignetteFilterTransformation(mContext, new PointF(0.5f, 0.5f),
                    new float[]{0.0f, 0.0f, 0.0f}, 0f, 0.75f), new CropSquareTransformation(mContext))
            .dontAnimate()
            .thumbnail(FILTER_IMAGE_LOADING_ALPHA)
            .into(imageView);
    String fileName = Type.VINETTE.getName();
