...
import com.sz.image.editor.util.crop.AspectRatioUtil;
import com.sz.image.editor.util.crop.HandleUtil;
import com.sz.image.editor.util.crop.PaintUtil;
import com.sz.image.editor.util.crop.cropwindow.edge.Edge;
import com.sz.image.editor.util.crop.cropwindow.handle.Handle;
...
public class CropImageView extends ImageView {
  ...
  /* ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ추가ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ*/
  public float getCropX() {
    ...
    return (bitmapLeft + Edge.LEFT.getCoordinate()) / scaleX;
  }
  ...
  public float getCropY() {
    ...
    return (bitmapTop + Edge.TOP.getCoordinate()) / scaleY;
  }

  public float getCropWidth(Bitmap bitmap) {
    float[] matrixValues = new float[9];
    getImageMatrix().getValues(matrixValues);
    float scaleX = matrixValues[Matrix.MSCALE_X];
    float transX = matrixValues[Matrix.MTRANS_X];
    ...
    return Math.min(Edge.getWidth() / scaleX, bitmap.getWidth() - cropX);
  }

  public float getCropHeight(Bitmap bitmap) {
    float[] matrixValues = new float[9];
    getImageMatrix().getValues(matrixValues);
    float transY = matrixValues[Matrix.MTRANS_Y];
    float bitmapTop = (transY < 0) ? Math.abs(transY) : 0;
    ...
    return Math.min(Edge.getHeight() / scaleY, bitmap.getHeight() - cropY);
  }
  /* ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ추가ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ*/
