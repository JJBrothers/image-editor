...
public class Sepia extends Filter{
  ...
  public Sepia(Context context,String targetPath, int width, int height){
    mContext = context;
  ...
  public void assignFilterImage(TextView textView, ImageView imageView) {
    ...
    Glide.with(mContext)
            ...
            .bitmapTransform(new SepiaFilterTransformation(mContext), new CropSquareTransformation(mContext))
            .dontAnimate()
            .thumbnail(FILTER_IMAGE_LOADING_ALPHA)
            .into(imageView);
    String fileName = Type.SEPIA.getName();
